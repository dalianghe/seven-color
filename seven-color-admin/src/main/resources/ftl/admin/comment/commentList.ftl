...
<div class="lenos-search">
  <div class="select">
    ...
    <div class="layui-inline">
      <input class="layui-input" height="20px" id="content" autocomplete="off">
    </div>
...
<table id="commentList" class="layui-hide" lay-filter="user"></table>
  ...
  layui.use('table', function () {
    ...
    table.render({
      ...
      , cols: [[
        ...
        , */{field: 'content', title: '内容', width: '52%'}
        , {field: 'createTime', title: '创建时间', width: '23%'}
    ...
    table.on('tool(user)', function (obj) {
      ...
      if (obj.event === 'detail') {
        ...
        layer.confirm('确定删除回帖[<label style="color: #00AA91;">' + data.content + '</label>]?', {
          btn: ['删除']
        }, function (index) {
          del(data.commentId, false, index);
        });
    ...
    $.ajax({
      ...
      success:function(d){
        if(d.flag){
          ...
          layui.table.reload('commentList');
        }else{
